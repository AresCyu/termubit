...
- **0.01 TRB per kilobyte** transaction fee
- **0.01 TRB** dust limit (discard threshold)
- **0.001 TRB** replace-by-fee increments
...
Note: In the past, Termubit has enforced a rounding function in the fee
      ...
      transaction only has to pay `0.01 / 1000 * 192 = 0.00192` TRB fee.

## Miner default inclusion policies

The default values for miners to include a transaction in a block has been set
to exactly the recommended fee of **0.01 TRB/kB.** Dust limits are defined by
...
The default minimum transaction fee for relay is set at **0.001 TRB/kB**,
...
- The hard dust limit is set at **0.001 TRB** - outputs under this value are
  invalid and rejected.
- The soft dust limit is set at **0.01 TRB** - sending a transaction with outputs
  under this value, are required to add 0.01 TRB for each such output, or else
