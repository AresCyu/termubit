...
Termubit network from 1 TRB to 0.01 TRB. The full recommendation can be found
...
1. The wallet will attempt to not create any outputs smaller than 0.03 TRB as
   change if possible.
2. If after signing there is more than 0.01 TRB left in the change output, the
   wallet will keep the output as-is.
3. If the change output drops under 0.01 TRB, the wallet will discard the
   change and instead give it to miners as fee.
4. Any output under 0.01 TRB will not be accepted as valid by default.
