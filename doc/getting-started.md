...
This tutorial will help you to go through the basics to use Termubit Core after you completed the [installation instructions](/INSTALL.md). You now have `termubitd` or `termubit-qt` executables available to run a node, and `termubit-cli`/`termubit-tx` tools to help you transact TRB.
...
The Termubit Core installation provides the `termubit-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `22555`, so that other tools and libraries can interact with it.
...
To receive TRB, you need an address that is securely derived from a private key through a series of automatic, cryptographic operations. The *address* can be shared with anyone to receive TRB, but the *private key* is sensitive information that allows anyone that knows it to spend the TRB on the associated address.
...
You now have two wallet addresses to share with other people to receive TRB! Consider avoiding [address reuse](https://en.bitcoin.it/wiki/Address_reuse) for anonymity and security reasons.
